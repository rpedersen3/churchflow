...
from quotesbot.utilities.groupextractorbeautifulsoup import GroupExtractorBeautifulSoup
...
class UpdateChurchWithGroupsFromWebPages:
    ...
    def appendWebPagesBasedOnGroup(self, church, startURLs):
    ...
    def updateWithGroupFromStaffWebPagesUsingChromeDriver(self, church):
        ...
        if "pages" in church:
            for page in church["pages"]:
                if "url" in page and "type" in page and page["type"] == "groups":

                    changed = True

                    url = page["url"]

                    processor = "extract-churchgroup-from-webpage-chromedriver"
                    needsToBeProcessed = self.helpers.checkIfNeedsProcessing(church, processor, url)

                    if needsToBeProcessed == True:

                        print("process groups page: ", url)
                        ...
                        extractor = GroupExtractorBeautifulSoup()
                        schema = extractor.constructSchema(church, url, soup)
                        extractor.extractGroupsUsingSchema(church, url, soup, schema)
